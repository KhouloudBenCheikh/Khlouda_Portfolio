...
const Nav = () => {
  ...
  return (
    ...
    >
      <div
        className='flex w-full xl:flex-col items-center justify-between xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[60px] xl:h-max py-6 ☐ bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full'
      >
        {navData.map((link, index) => (
          <Link
            className={`relative flex items-center group transition-all duration-300 ${
              link.path === pathname ? 'text-[#63d2e3]' : ''
            }`}
            href={link.path}
            key={index}
          >
